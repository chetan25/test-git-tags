    ...
    stages {
        ...
        stage('Trigger Job') {
            steps {
                ...
                curl -X POST -u "$JENKINS_TOKEN_USR:$JENKINS_TOKEN_PSW" http://localhost:9090/job/TestDraftPR/buildWithParameters?token=1234 --data VERSION=${REPO_LATEST_TAG} --data PR_NAME=${PR_NAME}
