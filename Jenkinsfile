        ...
        stage('Trigger Job') {
            steps {
                ...
                withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId:'jenkins-remote', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD']]) {
                    sh "curl -X POST -u $USERNAME:$PASSWORD http://localhost:9090/job/TestDraftPR/buildWithParameters?token=1234 --data-urlencode json='{"parameter": [{"name":"VERSION", "value":"2.1"}]}'"
                }
            }
        }
    }
